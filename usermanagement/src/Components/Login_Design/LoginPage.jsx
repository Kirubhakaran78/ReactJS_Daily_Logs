...
import {
  ...
  CF_sessionSet,
  CF_sessionGet,
} from "../Common/CookieSessionStorage/CookieFun";
import { useNavigate } from "react-router-dom";
import { useAuth } from "../Common/Context/AuthContext";
import SearchableSelect from "./SearchableSelect";
import { BsEye, BsEyeSlash } from "react-icons/bs";
import { FaPaperPlane } from "react-icons/fa"; // add to your imports

/* ---------------- Custom Modal ---------------- */
const CustomModal = ({ show, title, children, onClose, footer }) => {
  if (!show) return null;
  return (
...
const LoginPage = () => {
  ...
  // Static Time Zone options (common world zones)
  ...
  async function usernamepasswordcheck(data) {
    ...
    try {
      ...
      var rows = await postAPI(
        objdata,
        "Login/NewpasswordGenerationValidation",
        true
      );
      ...
      if (row.Resp.Sts == "1") {
        if (row.Resp.desc.TimeZones != undefined) {
          ...
          if (row.Resp.desc.TimeZones && row.Resp.desc.TimeZones.length > 0) {
            ...
            const matchedTimezone = row.Resp.desc.TimeZones.find(
              (tz) =>
            ...
            if (matchedTimezone) {
              setFormValues((prev) => ({
                ...prev,
                timezone: matchedTimezone.sTimeZoneID,
              }));
            } else {
              const asiaKolkataTimezone = row.Resp.desc.TimeZones.find(
                (tz) =>
                  tz.sTimeZoneValue && tz.sTimeZoneID.includes("Asia/Kolkata")
              );
              debugger;
              if (asiaKolkataTimezone) {
                console.log(
                  "Setting default timezone to Asia/Kolkata (UTC+05:30):",
                  asiaKolkataTimezone
                );
                setFormValues((prev) => ({
                  ...prev,
                  timezone: asiaKolkataTimezone.sTimeZoneID,
                }));
              }
            }
          }
  ...
  // put inside the LoginPage component (near other useEffect hooks)
  useEffect(() => {
    function focusHandler(e) {
      const el = e.target;
      // selector targets react-select text input inside our sdms-select prefix
      if (
        el &&
        el.matches &&
        el.matches(".sdms-select__input > input, input.form-input")
      ) {
        try {
          // move caret to the end
          const len = el.value ? el.value.length : 0;
          // for some browsers we need to set selection range:
          el.setSelectionRange && el.setSelectionRange(len, len);
          // fallback: blur + focus (rarely needed)
          // el.blur(); el.focus();
        } catch (err) {
          // swallow errors silently
          // console.warn('caret move failed', err);
        }
      }
    }
  ...
  useEffect(() => {
    ...
    if (domainsData.length > 0 && !formValues.domain) {
      ...
      if (domainValue) {
        console.log(
          "Fallback: Setting domain to first available:",
          domainValue
        );
        setFormValues((prev) => ({
          ...prev,
          domain: domainValue,
          sCategories: firstDomain.sCategories,
        }));
      }
    }
  ...
  async function loginservice(data) {
    ...
    try {
      ...
      if (row.Resp.Sts == "1") {
        ...
        let tempErrors = {};
        tempErrors.loginPassword =
          row.Resp.Msg || "Login failed. Please try again.";
        setErrors(tempErrors);
        setIsLoading(false);
      }
    } catch (error) {
  ...
  const handleSuccessfulLogin = (responseData, loginData) => {
    try {
      ...
      let tempErrors = {};
      tempErrors.loginPassword =
        "Error storing session data. Please try again.";
      setErrors(tempErrors);
    }
  };
  ...
  const setPostLoginValues = (
    ...
    try {
      ...
      CF_sessionSet(
        "ConcurrentUsersAvailableCount",
        userData.concurrentUsersAvailableCount,
        0
      );
      setCookie(
        "ConcurrentUsersAvailableCount",
      ...
      setCookie(
        "DownloadLinkLogoff",
        CF_encrypt(JSON.stringify(responseData.DownloadLinkLogoff.toString()))
      );
      ...
      setCookie(
        "LicensedOrNot",
        CF_encrypt(JSON.stringify(userDetails.sLicenseCode))
      );
      ...
      const categories =
        activeUserDetails.sCategories || formValues.sCategories || "DB";
      ...
      const domainName =
        activeUserDetails.sDomainName || formValues.domain || "SDMS";
      ...
      const siteName =
        sitesData.find((site) => site.sSiteCode === siteCode)?.sSiteName || "";
      ...
      const timeZoneValue =
        timezonesData.find((tz) => tz.sTimeZoneID === timeZoneId)
          ?.sTimeZoneValue || "";
      ...
      // sUsername - encrypted
      const username =
        activeUserDetails.sUsername || formValues.loginUser || "";
      ...
      CF_sessionSet(
        "ActiveUserDetails",
        JSON.stringify(activeUserDetailsObj),
        0
      );
      setCookie("ActiveUserDetails", JSON.stringify(activeUserDetailsObj));

      console.log(
        "Post-login session storage and cookie values set successfully"
      );
    } catch (error) {
      debugger;
  ...
  const handleSendEmail = () => {
    console.log("Sending email to:", forgotEmail);
  ...
  const handleSubmit = (e) => {
    ...
    if (validate()) {
      ...
      let passobj = {
        ...
        sAvailableLicenseURL:
          "http://AGL66:8095/LicenseAPI/License/getAvailableLicenseCount",
      };
  ...
  const handleforgotpassword = (e) => {
    e.preventDefault();
    console.log("Forgot password clicked, current passOpen:", passOpen);
    setPassOpen(true);
    console.log("Setting passOpen to true");
  };
  ...
  return (
    <div
      className={`login-container-simple ${showMoreInfo ? "more-open" : ""}`}
    >
      ...
      <div className="login-form-box">
        <div className="login-header">
          <div className="logo-container">
            ...
            >
              <img src="./SDMS_Logo.png" alt="SDMS Logo" />
              <span
                id="login_versionno"
                className="login_versionno"
                style={{
                  fontSize: "10px",
                  fontWeight: "bold",
                  color: "green",
        ...
        <form onSubmit={handleSubmit} className="login-form">
          <div className="form-grid">
            ...
            <div className="form-field">
              ...
              <div
                className={`password-input-container ${formValues.loginPassword ? "has-value" : ""
                  }`}
              >
                <input
                ...
                {/* Only render button when there's a value */}
            ...
            <div className="more-info-section">
              <div className="form-field">
                ...
                <SearchableSelect
                  ...
                  onChange={(val) =>
                    setFormValues((prev) => ({ ...prev, site: val }))
                  }
              ...
              <div className="form-field">
                ...
                <SearchableSelect
                  ...
                  onChange={(val) =>
                    setFormValues((prev) => ({ ...prev, domain: val }))
                  }
              ...
              <div className="form-field">
                ...
                <SearchableSelect
                  ...
                  onChange={(val) =>
                    setFormValues((prev) => ({ ...prev, timezone: val }))
                  }
              ...
              <div className="form-field">
                ...
                <SearchableSelect
                  options={languagesDatas.map((l) => ({
                    value: l.language,
                    label: l.language,
                  }))}
                  value={formValues.language}
                  onChange={(val) =>
                    setFormValues((prev) => ({ ...prev, language: val }))
                  }
                  placeholder="Select Language"
                  isSearchable={
                    false
                  } /* optional: language list small, no need to search */
                  isClearable={false}
      ...
      <CustomModal
        ...
        onClose={() => { setPassOpen(false); setForgotEmail(""); }}
        footer={
          <>
            ...
            <button
              type="button"
              className="btn btn-primary custom-modal-send-btn"
            ...
            <button
              type="button"
              className="btn btn-outline-secondary custom-modal-footer-btn"
              onClick={() => { setPassOpen(false); setForgotEmail(""); }}
            >
              Close
            </button>
          </>
      ...
      <div id="login_documentfilesimages">
        <div className="circle-wrap">
          {/* <!-- dashed ring --> */}
          <div className="dashed-ring" aria-hidden="true"></div>

          {/* <!-- top --> */}
          <div className="circle-logo icon-top">
            <img src="documentfiles.png" alt="Documents & Files" />
            <span className="icon-label">Documents & Files</span>
          </div>

          {/* <!-- right --> */}
          <div className="circle-logo icon-right">
            <img src="application-gateway.png" alt="CFR Gateway" />
            <span className="icon-label">CFR Gateway</span>
          </div>

          {/* <!-- bottom --> */}
          <div className="circle-logo icon-bottom">
            <img src="dataOrientedApp.png" alt="Database Application" />
            <span className="icon-label">Database Application</span>
          </div>

          {/* <!-- left --> */}
          <div className="circle-logo icon-left">
            <img src="instrumentdata.png" alt="Instrument Data" />
            <span className="icon-label">Instrument Data</span>
          </div>

          {/* <!-- center big logo --> */}
          <div className="center-logo">
            <img src="Logilab SDMS_vertical.png" alt="Logilab SDMS" />
          </div>
        </div>
      </div>
    </div>
